...
////////////////////////////////////////////////////////////////////////
	...
	// HHH controll Socket 
	struct sockaddr_in hhh_their_addr;
	
	// Init global variables
	hhh_sockfd = 0;
	hhh_job_counter = 0;
		
	if ((hhh_sockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
		ha_warning("HHH socket failed");
	}
    		
	hhh_their_addr.sin_family = AF_INET;      
	hhh_their_addr.sin_port = htons(9091);   
	hhh_their_addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
	bzero(&(hhh_their_addr.sin_zero), 8);    
	hhh_timer = time(NULL);
	
	while (connect(hhh_sockfd, (struct sockaddr *)&hhh_their_addr, \
										  sizeof(struct sockaddr)) == -1) {
		if (time(NULL) - hhh_timer > 1) {
			ha_warning("HHH socket connect failed after %ld seconds\n", time(NULL) - hhh_timer);
			ha_warning("HHH working in a single LB mode\n");
			hhh_sockfd = -1;
			break;
		}		
	}
	...
	window_hh_init(windowSize, 0.01);
	wrhhh1d_init(0.01, 1, windowSize, 0, 1);
