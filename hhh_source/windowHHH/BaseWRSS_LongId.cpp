...
BaseWRSS_LongId::BaseWRSS_LongId(unsigned int windowSize, float gamma, unsigned int m, float epsilon)
{
    overflowsNumber = 0;
    tail = 0;
    indexTail = 0;
    ...
    index = new vector<char> (indexSize); // 0 means end of block
    overflowsElements = new ID[maxOverflows];
    rss = new RSS_CPP_LongId(epsilon / 4., m, gamma);//y
...
BaseWRSS_LongId::~BaseWRSS_LongId()
{
...
void BaseWRSS_LongId::incTimestamp(){
	...
	ID oldId = overflowsElements[tail];
    try {
		//printf("1 ");
...
void BaseWRSS_LongId::update(ID item, int weight)
{
	//printf("in update\n");
	incTimestamp();
...
unsigned int BaseWRSS_LongId::query(ID item)
{
    int minOverFlows;
...
/*
void BaseWRSS_LongId::sampleUpdate(unsigned int item, int weight, int timeElapsed){
