...
RSS_CPP_LongId::RSS_CPP_LongId(float fPhi, int M, float gamma)
{
	int i;
	...
	m_hashtable = (RSSITEM_LongId**)calloc(m_tblsz, sizeof(RSSITEM_LongId *));
	m_groups = (RSSGROUP_LongId *)calloc(m_nrCounters, sizeof(RSSGROUP_LongId));
	m_items = (RSSITEM_LongId *)calloc(m_nrCounters, sizeof(RSSITEM_LongId));
	m_freegroups = (RSSGROUP_LongId **)calloc(m_nrCounters, sizeof(RSSGROUP_LongId*));
...
//RSSGROUP_LongId *dbg=NULL;

void RSS_CPP_LongId::update(ID newitem, int weight) {
	/*if (dbg){
	...
	RSSITEM_LongId *til = getCounter(newitem, h);
	RSSITEM_LongId &il = (til ? *til : takoverMinimal(newitem, h));
	/*if (dbg){
...
inline int const RSS_CPP_LongId::getHash(ID item) {
	return hash31(m_a, m_b, item) % m_tblsz;
}

inline RSSITEM_LongId * RSS_CPP_LongId::getCounter(ID item, int h) {
	RSSITEM_LongId * il = m_hashtable[h];
	while (il && (il->item != item))
		il = il->nexti;
	return il;
}

inline RSSITEM_LongId & RSS_CPP_LongId::takoverMinimal(ID item, int h) {
	RSSITEM_LongId & il = *(m_root->items);
	removeFromHash(il);
...
void RSS_CPP_LongId::advanceCounter(RSSITEM_LongId &il, unsigned int nrIncs) {
	//RSSGROUP_LongId *g = il.parentg;
	/*if (((RSSGROUP_LongId *)0x791050)->count == 0)
		printf("zerrrrro\n");*/
	/*if (g->count >= 807){
		while (g->nextg){
	...
	if (g->count >= 1){
		while (g){
			...
			if (!(g->count)){
				...
				RSSITEM_LongId *til = getCounter(23106048, h);	
			}
			g = g->nextg;
		}
	}*/
	RSSGROUP_LongId &oldgroup = *(il.parentg);
	unsigned int goalVal = oldgroup.count + nrIncs;
	RSSGROUP_LongId &group = getLastGroup(oldgroup, goalVal);
...
inline void RSS_CPP_LongId::disconnectCounter(RSSITEM_LongId & il) { //disconnect only if there exists other counters in the same group
...
inline RSSGROUP_LongId & RSS_CPP_LongId::getLastGroup(RSSGROUP_LongId & oldgroup, unsigned int goalVal) {
	RSSGROUP_LongId *group = &oldgroup;
...
inline void RSS_CPP_LongId::removeFromHash(RSSITEM_LongId & il) {
...
inline void RSS_CPP_LongId::addToGroup(RSSGROUP_LongId & group, RSSITEM_LongId & il) {
...
inline void const RSS_CPP_LongId::showGroups() {
	RSSGROUP_LongId *g;
	RSSITEM_LongId *i, *first;
	int n, wt;
...
void RSS_CPP_LongId::insertIntoHashtable(RSSITEM_LongId &newi, int i, ID newitem) {
...
unsigned int const RSS_CPP_LongId::query(ID x) {
	/*if (dbg){
...
inline void RSS_CPP_LongId::connectToGroup(RSSITEM_LongId &newi, RSSGROUP_LongId &tmpg) {
...
inline void RSS_CPP_LongId::putInNewGroup(RSSITEM_LongId &newi, RSSGROUP_LongId & tmpg) {
	RSSGROUP_LongId * oldgroup = newi.parentg;
...
inline void RSS_CPP_LongId::moveGroup(RSSITEM_LongId &il, RSSGROUP_LongId &group, unsigned int goalVal) {
	...
	RSSGROUP_LongId &tmpg = *(il.parentg);
...
inline void RSS_CPP_LongId::newGroup(RSSITEM_LongId &il, RSSGROUP_LongId &group, unsigned int goalVal) {
	assert(group.count < goalVal);
	//std::cout << m_gpt << ',' << m_n << std::endl;
	assert(il.parentg->items != &il);
	// Create a new group for newi
	// RSS_AddNewGroupAfter(rss, newi,group);
	RSSGROUP_LongId &newgroup = *(m_freegroups[m_gpt++]); //get new group
